...
class CoreDataImageDataStoreTests: XCTestCase {
    ...
    func test_deleteWithURL_deliversNoErrorOnNonEmptyCache() {
        let sut = makeSUT()
        let exp = expectation(description: "wait for completion")
        let url = anyURL()
        
        insertError(to: sut, with: url, data: anyData())

        var capturedError: Error?
        sut.deleteImageData(with: url) {
            capturedError = $0
            exp.fulfill()
        }
    ...
    func test_deleteWithURL_emptiesImageURLData() {
        let sut = makeSUT()
        let exp = expectation(description: "wait for completion")
        let url = anyURL()
        ...
        sut.deleteImageData(with: url) { _ in
            exp.fulfill()
        }
        ...
        XCTAssertEqual(capturedData, nil)
    }
